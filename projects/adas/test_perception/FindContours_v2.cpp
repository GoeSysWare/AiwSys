...
void FindContours_v2::load_params() {
	...
	std::ifstream file(FLAGS_calibrator_cfg_distortTable, std::ios::in | std::ios::binary | std::ios::ate);
	if (file.is_open())
	{
...
void FindContours_v2::OnPointCloud(const watrix::proto::PointCloud& data, watrix::proto::PointCloud& lidar2image_paint_,
	watrix::proto::PointCloud& lidar_safe_area_, std::vector<cv::Point3f>& lidar_cloud_buf, int& effect_point)
{
	...
	long lidar_mem_length = data.points_size() * 3;
	...
	for (int i = 0; i < data.points_size(); i++) {
		lidar_buffer[p_count++] = data.points(i).x();
		lidar_buffer[p_count++] = data.points(i).y();
		lidar_buffer[p_count++] = data.points(i).z();
		lidar_cloud_buf.push_back(cv::Point3f(data.points(i).x(), data.points(i).y(), data.points(i).z()));  //return
	}
	...
	cv::Mat lidar_points = cv::Mat(data.points_size(), 3, CV_64FC1, lidar_buffer);
	...
	for (int w = 0; w < WIDTH; w++) {	
		...
		*(imagexy_check + cv_ori_point_i * image_width +cv_ori_point_j)  = 1;
		watrix::proto::LidarPoint* lidar_point1 = lidar2image_check_.add_points();
		lidar_point1->set_x(cv_ori_point_j);
		lidar_point1->set_y(cv_ori_point_i);
		//////////////////////////////////////////////
		watrix::proto::LidarPoint* lpt1 = lidar2image_paint_.add_points();
		...
		//safe area data 
		watrix::proto::LidarPoint* lpt2 = lidar_safe_area_.add_points();
		lpt2->set_x(data.points(w).x());
		lpt2->set_y(data.points(w).y());
		lpt2->set_z(data.points(w).z());
...
std::vector<cv::Point2d> FindContours_v2::start_contours(std::vector<watrix::proto::LidarPoint> points) {
...
// 	FindContours_v2::start_contours(points);
