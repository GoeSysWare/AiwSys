...
namespace apollo
{
  namespace dreamview
  {
   ...
   template <>
    void ImageHandler::OnImage(const std::shared_ptr<watrix::projects::adas::proto::SendResult> &image)
    {
      if (requests_ == 0)
      {
        return;
      ...
      cv::Mat mat(image->source_image().height(), image->source_image().width(), CV_8UC3,
                  const_cast<char *>(image->source_image().data().data()), image->source_image().step());
      // cv::cvtColor(mat, mat, cv::COLOR_RGB2BGR);
      cv::resize(
          mat, mat,
          cv::Size(static_cast<int>(image->source_image().width() ),
                   static_cast<int>(image->source_image().height() )),
          0, 0, CV_INTER_LINEAR);
    ...
    void ImageHandler::OnImageResult(
        const std::shared_ptr<watrix::projects::adas::proto::SendResult> &image)
    {
      if (!FLAGS_use_navigation_mode)
      {
        ...
        : requests_(0), node_(cyber::CreateNode("image_handler"))
    {
      ...
      //     "adas/camera/front_6mm/image",
      //     [this](const std::shared_ptr<Image> &image) { OnImageFront(image); });

      // node_->CreateReader<CompressedImage>(
      //     "adas/camera/front_6mm/image/compressed",
      //     [this](const std::shared_ptr<CompressedImage> &image) {
      //       OnImageShort(image);
      //     });
      node_->CreateReader<watrix::projects::adas::proto::SendResult>(
          "adas/camera/front_6mm/result",
          [this](const std::shared_ptr<watrix::projects::adas::proto::SendResult> &image) { OnImageResult(image); });
    }
