...
#define INDEX_LIDAR 2
      ...
      //封装image 注意两个frame_id在sim模式下的区别和用途
      static void makeProtoImageFormMat(cv::Mat &in_Mat,
                                        std::string frameId,
                                        std::string &model_name,
                                        std::shared_ptr<apollo::drivers::Image> out_Image)
      {
        ...
        // 利用头定义frame_id 和sequence_num 确定一个模块
        out_Image->mutable_header()->set_timestamp_sec(apollo::cyber::Time::Now().ToSecond());
        out_Image->mutable_header()->set_sequence_num(AdasSimulatorComponent::procs_num_.load());
        out_Image->mutable_header()->set_frame_id(frameId);
        
        
        //仿真模式下frame_id可以定义为文件名
        out_Image->set_frame_id(model_name);
      ...
      output_lidar_channel_names:   %s)";
        std::string config_info_str =
            boost::str(boost::format(format_str.c_str()) % sim_files_dir_ % sim_config_file_ % this->GetInterval() % output_camera_channel_names_[0] % output_camera_channel_names_[1] % output_lidar_channel_names_[0]);
      ...
      lidar file:  %s)";
        std::string config_info_str =
            boost::str(boost::format(format_str.c_str()) % procs_num_.load() % (*cur_Iter_)[INDEX_6MM] % (*cur_Iter_)[INDEX_12MM] % (*cur_Iter_)[INDEX_LIDAR]);
        AINFO << config_info_str;
      ...
      void AdasSimulatorComponent::ParseCameraFiles(std::string file_6mm, std::string file_12mm)
      {
        ...
        makeProtoImageFormMat(mat_6mm, "sim_6mm_camera", file_6mm, front_6mm_image_);
        ...
        makeProtoImageFormMat(mat_12mm, "sim_12mm_camera", file_12mm, front_12mm_image_);
      }
      ///解析雷达文件的
      void AdasSimulatorComponent::ParseLidarFiles(std::string file_lidar)
      {
        ...
        pcd_file.close();
          ...
          // 利用头定义frame_id 和sequence_num 确定一个模块
        lidar_pointcloud_->mutable_header()->set_timestamp_sec(apollo::cyber::Time::Now().ToSecond());
        lidar_pointcloud_->mutable_header()->set_sequence_num(AdasSimulatorComponent::procs_num_.load());
        lidar_pointcloud_->mutable_header()->set_frame_id("sim_lidar");
        
        
        //仿真模式下frame_id可以定义为文件名
        lidar_pointcloud_->set_frame_id(file_lidar);
        //仿真里面的测量时间以当前时间计
