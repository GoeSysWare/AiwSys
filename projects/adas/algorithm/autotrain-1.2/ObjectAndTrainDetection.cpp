...
namespace watrix {
	namespace algorithm {
		...
		pcl::PointCloud<pcl::PointXYZ>::Ptr LOD::getPointFrom2DAnd3D(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, std::unordered_map<int, std::pair<int, int>> invasionP,
			...
			cv::Mat cameraMatrix(3, 3, CV_32F);//相机的内参矩阵
			std::vector<float> distCoeff;//相机畸变参数
			cv::Mat rvec(3, 3, CV_64F), tvec(3, 1, CV_64F);//变换矩阵
			...
			//7月10日雷达旋转参数,通过标定得出
			rotation1 << 0.998985, 0.000149, -0.045041, 0.000000,     //雷达坐标系到世界坐标系的变换，高度没变
				0.000149, 0.999978, 0.006600, 0.000000,
				0.045041, -0.006600, 0.998963, 0.000000,
				0.000000, 0.000000, 0.000000, 1.000000;

			rotation2 << 1, 0, 0, 0,                                  //雷达水平方向旋转矩阵 
			...
			for (int i = 0; i < cloud->points.size(); ++i) {
				...
				//当拟合的限界发散时，以限界中点为基准，重新向左右扩展限界,缩小了限界
				double virtual_mid;
		...
		float LOD::calHeightVar(std::vector<Point3> object) {  //计算一个目标的高度方差，为了判断其是否是一个平面，进行滤除
		...
		FitLineData LOD::fitNextPoint(std::vector<cv::Point2d> lines, float row_radio) {
			...
			if (temp = (lines.size() * A - B * B)) {// 判断分母不为0
		...
		void LOD::calLowestPoint(TrackData& trackdata, pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud, pcl::PointXYZ min_p,
			...
			for (int i = 0; i < GROUND_JUDGE; ++i) {
				...
				trackdata.trackBottomFit[i] = trackdata.trackTop[i] - 0.25f;     //25cm是隧道内的轨面轨底差值

			}

			//当判断在车库内、两轨之间有坑的情况下，执行以下过程
			if (isHole) {
		...
		std::vector<LidarBox> LOD::object_detection(pcl::PointCloud<pcl::PointXYZ>::Ptr& points, Eigen::Matrix4f rotation, InvasionData invasion, std::string image_file) {
			...
			float col_radio = 0.2f;   //0.05   阈值调整
			int grid_row = (max_p.z - min_p.z) / row_radio + 1;
			...
			for (int i = 0;i < obstacle_grid.size();) {
				...
				if (trackBottomAvr - loss_min_x < 0.08f) {     //高度阈值
					++i;
					continue;
				}
				else {
