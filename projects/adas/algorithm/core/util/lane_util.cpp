...
#pragma region get lane mask framework
			void LaneUtil::cvHilditchThin1(cv::Mat& src, cv::Mat& dst)
			{
				if(src.type()!=CV_8UC1)
				{
					printf("只能处理二值或灰度图像\n");
...
#pragma endregion
			...
			)
			{
				...
				for(int i=0; i< size; i++){
					...
					// Mat result(lane_image.size(), CV_8UC1, Scalar::all(0));

					// get lane image framwork to lane_image_copy
					cv::Mat lane_image_copy = lane_image.clone();
					// std::cout << "lane_image size:" << lane_image.size() << std::endl;
					const int col_num = lane_image_copy.cols;
        			const int rows_num = lane_image_copy.rows;
					copyMakeBorder(lane_image_copy, lane_image_copy, 1, 1, 1, 1, BORDER_CONSTANT, Scalar::all(0));
					cvHilditchThin1(lane_image_copy, lane_image_copy);
					lane_image_copy = lane_image_copy(Rect(1,1,col_num,rows_num));
					// std::cout << "lane_image_copy size:" << lane_image_copy.size() << std::endl;

					// Mat image_result(lane_image_copy.size(), CV_8UC1, Scalar::all(0));
					// Mat_<uchar> tmp = lane_image_copy;
					// for (int x=0; x<lane_image_copy.cols; x++){
					// 	for (int y=0; y<lane_image_copy.rows; y++){
					// 		if (tmp(Point(x,y)) != 0)
					// 			image_result.at<uchar>(Point(x,y)) = 255;
					// 	}
					// }
					// cv::imwrite("./gudian"+std::to_string(i)+"_.png", image_result);

					for(int r=0; r<lane_image.rows;r++){
						for(int c=0; c<lane_image.cols;c++){
							if (lane_image_copy.at<uchar>(r,c) == 1) {
								// std::cout << "lane_image_copy.at<uchar>(r,c):" << int(lane_image_copy.at<uchar>(r,c)) << std::endl;
								dpoint_t pointxy;
