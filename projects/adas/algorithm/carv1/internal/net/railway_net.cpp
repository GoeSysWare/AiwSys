...
#pragma region init and free for RailwayCropNet
			
			std::vector<shared_caffe_net_t> RailwayCropNet::v_net;

			void RailwayCropNet::init(
			...
			void RailwayCropNet::free()
			{
				CaffeNet::free(v_net);
			}

			void RailwayCropNet::get_inputs_outputs(
...
#pragma region init and free for RailwayDetectNet

			std::vector<shared_caffe_net_t> RailwayDetectNet::v_net;

			void RailwayDetectNet::init(
				const caffe_net_file_t& caffe_net_file
			)
			{
				CaffeNet::init(caffe_net_file, net_count, v_net);
			...
			void RailwayDetectNet::free()
			{
				CaffeNet::free(v_net);
			}

			void RailwayDetectNet::get_inputs_outputs(
				CaffeNet::caffe_net_n_inputs_t& n_inputs,
				CaffeNet::caffe_net_n_outputs_t& n_outputs
			)
			{
