...
PyObject *cyber_PyRecordReader_GetProtoDesc(PyObject *self, PyObject *args) {
  ...
  return PyBytes_FromStringAndSize(pb_desc.c_str(), pb_desc.size());
}
...
PyObject *cyber_PyRecordReader_GetHeaderString(PyObject *self, PyObject *args) {
  ...
  return PyBytes_FromStringAndSize(header_string.c_str(),
                                    header_string.size());
}
...
PyObject *cyber_PyRecordWriter_GetProtoDesc(PyObject *self, PyObject *args) {
  ...
  return PyBytes_FromStringAndSize(proto_desc_str.c_str(),
                                    proto_desc_str.size());
}
...
//   // python2 的自定义模块C语言加载方法
//   Py_InitModule("_cyber_record", _cyber_record_methods);
// }


static struct PyModuleDef cyber_record =
{
    PyModuleDef_HEAD_INIT,
    "_cyber_record", /* name of module */
    "",          /* module documentation, may be NULL */
    -1,          /* size of per-interpreter state of the module, or -1 if the module keeps state in global variables. */
    _cyber_record_methods
};

PyMODINIT_FUNC PyInit__cyber_record(void)
{
   AINFO << "init _cyber_record";
    return PyModule_Create(&cyber_record);
}